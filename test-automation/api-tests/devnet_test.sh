...
if [ "$doStaking" == "true" ]; then
    python3 test.py --test_dir=./tests/no-explorer/ --rpc_endpoint_src="https://api.s0.pga.hmny.io/" \
        --rpc_endpoint_dst="https://api.s1.pga.hmny.io/" --keystore=./DevnetValidatorKeys/ \
        --chain_id="pangaea" --delay=${delay} --iterations=${iters} --cli_passphrase='harmony-one'
else
    python3 test.py --test_dir=./tests/no-explorer/ --rpc_endpoint_src="https://api.s0.pga.hmny.io/" \
        --rpc_endpoint_dst="https://api.s1.pga.hmny.io/" --keystore=./DevnetValidatorKeys/ \
        --chain_id="pangaea" --delay=${delay} --iterations=${iters} --cli_passphrase='harmony-one' --ignore_staking_test
fi
